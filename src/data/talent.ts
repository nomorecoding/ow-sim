...
import {
  GROWTH_CAP, GROWTH_SEASONS_CAP, GROWTH_TRAINING_CAP, RETIRE_AGE, TALENT_INFO, TALENT_ORDER,
} from './constants'
...
export function growthPoints(g: Growth, age: number): number {
  // 赛季经验每 2 季 +1
  let p = Math.min(GROWTH_SEASONS_CAP, Math.floor(g.seasons / 2)) + g.heroPool + g.gear + Math.min(GROWTH_TRAINING_CAP, g.training)
  // 年龄：25 岁后每年 -1，30 岁后每年再 -1
  if (age >= RETIRE_AGE) p -= age - RETIRE_AGE + 1
...
/** 摇本季天赋：返回档位与隐藏 MMR。floor 为硬兜底（职业选手有战队训练环境，不会掉回木桶） */
export function rollTalent(points: number, floor?: TalentTier): { tier: TalentTier; mmr: number } {
  let tier = rollTier(points)
  if (floor && TALENT_ORDER.indexOf(tier) < TALENT_ORDER.indexOf(floor)) tier = floor
