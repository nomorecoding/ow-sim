...
import {
  DEFAULT_SPEED, FORM_INFO, FORM_ORDER, INTL_NAME, MAJOR_NAME, MIN_SPEED, QUIT_AGE, RANK_COLOR_CLASS, SCOUT_MAX_AGE,
...
import { beginLife, commitLife, createLife, freshMeta, lifeChoose, lifeStep, loadMeta, recountHeroPool, writeMeta } from './sim/life'
import { clamp, scoreToRank } from './sim/rank'
import { beginYear, commitYear, exposureP, formProbs, proChoose, proStep, teamOf, teamRating, yearInProgress } from './sim/pro'
...
function hideSheets() {
  for (const id of ['pause-mask', 'choice-mask', 'pro-pause', 'pro-choice']) { const m = $(id); if (m) m.hidden = true }
}
...
function talentPanel() {
  ...
  return `<div class="section">
    <div class="row" style="border-bottom:0;padding:0 0 6px"><span class="label" style="margin:0">下一辈子的天赋概率</span><span class="tip">成长 <b class="num">${pts}</b></span></div>
    ${rows}
    <p class="tip" style="margin:12px 0 0">成长 = 人生 ${Math.min(12, gr.runs)} · 英雄池 ${gr.heroPool}（${achN} 成就，每 5 个 +1）· 里程碑 ${gr.milestones}<br>天赋一辈子不变，决定实力涨多快、撞墙多好过。见过的怪事越多，下一辈子越好投胎。</p>
  </div>`
}
...
function renderHome() {
  ...
  const banBanner = p.lifetimeBan
    ? `<div class="mute-line on" style="margin-bottom:14px"><span>本存档职业生涯已永闭 · 终身禁赛</span><small>天梯人生照常，只是再没有人会私信你。想再打职业只能删档重来（成就保留）。</small></div>`
    : ''
  ...
  app.innerHTML = `<div class="reveal">
    ...
    <div class="dossier">
      ...
      <div class="cell"><div class="label">历史最高</div><div style="font-family:var(--display);font-size:1.05rem;line-height:1.6">${best}</div><small>被发掘 ${meta.scoutedTimes} 次</small></div>
      <div class="cell"><div class="label">成就</div><div class="num">${achCount}<span style="font-size:0.5em;color:var(--bone-dim)"> / ${ACHIEVEMENTS.length}</span></div><small>${meta.bestTalent ? `最好天赋 ${TALENT_INFO[meta.bestTalent].name}` : ''}</small></div>
    </div>
    ${proNow}
    ${talentPanel()}
  ...
  $('btn-wipe')!.onclick = () => {
    if (!confirm('删档重来：成长、里程碑、职业生涯、黑历史全部清零，成就与结局收集保留。确定？')) return
...
function renderAchievements() {
  ...
  app.innerHTML = `<div class="reveal">
    ${top.html}
    <p class="tip">每 5 个成就 → 英雄池 +1 → 下一辈子天赋分布上移。成就不改这辈子。✦ 为职业生涯荣誉。</p>
...
function refreshHud() {
  ...
  const extra = [
    g.goldGuns ? `<span>金枪 <b class="num">${g.goldGuns}</b></span>` : '',
    g.gear ? `<span>设备 <b class="num">${g.gear}</b></span>` : '',
    g.crewSeasons ? `<span>车队 <b class="num">${g.crewSeasons}</b></span>` : '',
    g.dirty.boostJobs + g.dirty.hires + g.dirty.cheatSeasons ? `<span class="warn">黑历史 <b class="num">${g.dirty.boostJobs + g.dirty.hires + g.dirty.cheatSeasons}</b></span>` : '',
  ].join('')
...
function renderLife() {
  ...
  $('btn-exit')!.onclick = () => { if (confirm('放弃这辈子：不写档、不计成长。确定？')) { life = null; renderHome() } }
...
function showChoice() {
  ...
  $('choice-btns')!.querySelectorAll<HTMLButtonElement>('[data-opt]').forEach((b) => {
    b.onclick = () => {
      ...
      if (id === 'cheat' && !confirm('开挂：过墙容易，封号一半。号没了这辈子就到此为止。确定？')) return
      lifeChoose(life!, id)
      hideSheets()
      stopTimer()
      tickLife()
...
function renderLifeSettle() {
  ...
  if (!committed) {
    committed = true
    toPro = commitLife(g, meta)
    writeMeta(meta)
  } else toPro = g.scouted
  ...
  const head = toPro
    ...
    : `<div class="sub" style="margin:0 0 6px">这一辈子 · ${g.age} 岁${g.banned ? '封号' : '退坑'}</div>
  ...
  app.innerHTML = `<div class="reveal">
    ...
    <div class="dossier">
      ...
      <div class="cell"><div class="label">阶段</div><div style="font-family:var(--display);font-size:1rem;line-height:1.5">${STAGE_INFO[g.stage].name}</div><small>${g.dirty.boostJobs + g.dirty.hires + g.dirty.cheatSeasons ? `黑历史 ${g.dirty.boostJobs + g.dirty.hires + g.dirty.cheatSeasons}` : '干净'}</small></div>
    </div>
    ${toPro ? `<div class="section ending-card">
        ...
        <div class="verse">这辈子的天赋会带进去：${TALENT_INFO[g.talent].name}。现金、人气、${g.dirty.boostJobs + g.dirty.hires + g.dirty.cheatSeasons ? '<span class="warn">还有黑历史</span>' : '干净的档案'}也一起。</div>
      </div>` : ''}
...
function formPanel(age: number) {
  ...
  return `<div class="section">
    <div class="row" style="border-bottom:0;padding:0 0 6px"><span class="label" style="margin:0">本年状态概率</span><span class="tip">职业成长 <b class="num">${p.growth}</b>${p.talentBonus ? ` · 天赋 ${p.talentBonus > 0 ? '+' : ''}${p.talentBonus}` : ''} · ${age} 岁${age >= 25 ? ' <span class="warn">年龄开始抵消</span>' : ''}</span></div>
    ${rows}
...
function renderAbout() {
  ...
  app.innerHTML = `<div class="reveal">
    ...
    <div class="section">
      ...
      <p class="tip" style="color:var(--bone)">天赋开局摇一档，一辈子不变，决定隐藏实力涨多快。每个大段之间有一道墙：实力顶到墙就要过检定，过不去就卡在 X1·9x——差一分的那种。卡久了会有一次抉择：练枪、换设备，或者继续打。</p>
    </div>
    <div class="section">
    ...
    <div class="section">
      ...
      <p class="tip" style="color:var(--bone)">${SCOUT_MAX_AGE} 岁前打到宗师以上，有概率收到青训教练的私信。试训通过就进职业：OWCS 中国赛区，一年一局，三个 Stage 逐场滚动，年末转会窗自己选队。天赋、现金、人气和黑历史都带过去。</p>
    </div>
    <div class="section">
      <div class="label">全局成长</div>
      <p class="tip" style="color:var(--bone)">每辈子打完，成长点 +1；每 5 个成就英雄池 +1；首次触及各大段 / 首次被发掘各 +1。成长只改下一辈子摇到高档天赋的概率，不改这辈子。</p>
    </div>
    <div class="section">
